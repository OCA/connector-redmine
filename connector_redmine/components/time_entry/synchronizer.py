...
class TimeEntryBatchImportSynchronizer(Component):
    ...
    def run(self, filters=None, options=None):
        ...
        updated_from = datetime.strptime(
            self.backend_record.time_entry_last_update, DEFAULT_SERVER_DATETIME_FORMAT
        )
        record_ids = self.backend_adapter.search(updated_from, filters)
...
class TimeEntryImportSynchronizer(Component):
    ...
    def run(self, record_id, options=None):
        ...
        if self.updated_on > backend.time_entry_last_update and not options.get(
            "single_user", False
        ):
